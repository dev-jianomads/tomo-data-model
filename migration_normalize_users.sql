...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'dev' AND table_name = 'users') THEN
        RAISE EXCEPTION 'dev.users table not found. Aborting migration.';
    END IF;
    
    -- Check if tables already exist (idempotency check)
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'dev' AND table_name = 'users_old') THEN
        RAISE WARNING 'dev.users_old already exists. This migration may have been run before.';
    END IF;
...
CREATE TABLE IF NOT EXISTS dev.users_new (
    -- Core identity
    id text PRIMARY KEY,
...
COMMENT ON TABLE dev.users_new IS 'New users table with clean schema (integration data removed)';

-- Copy core user data from old table
INSERT INTO dev.users_new (
    id,
    created_at,
    email,
    display_name,
...
DECLARE
    row_count int;
BEGIN
    SELECT count(*) INTO row_count FROM dev.users_new;
...
COMMENT ON TABLE dev.services IS 'Catalog of available integration services (Gmail, Telegram, etc.)';
COMMENT ON COLUMN dev.services.id IS 'Service identifier (e.g., gmail, telegram)';
COMMENT ON COLUMN dev.services.type IS 'Service category: email, calendar, messaging, storage, other';
COMMENT ON COLUMN dev.services.provider IS 'Provider/vendor: google, microsoft, telegram, signal, etc.';

-- 3.2: Create user_integrations junction table
-- Note: References old users table for now, will update after rename
CREATE TABLE IF NOT EXISTS dev.user_integrations (
    -- Primary key
    ...
    service_id text NOT NULL REFERENCES dev.services(id) ON DELETE RESTRICT,
...
COMMENT ON TABLE dev.user_integrations IS 'Links users to their connected services with credentials';
COMMENT ON COLUMN dev.user_integrations.user_id IS 'Reference to user who owns this integration';
COMMENT ON COLUMN dev.user_integrations.service_id IS 'Reference to service being integrated';
COMMENT ON COLUMN dev.user_integrations.external_user_id IS 'User identifier on external service (telegram_id, signal_uuid, etc.)';
COMMENT ON COLUMN dev.user_integrations.credentials IS 'Service-specific credential data (jsonb for flexibility)';
...
DECLARE
    service_count int;
BEGIN
    SELECT count(*) INTO service_count FROM dev.services;
...
BEGIN
    SELECT count(*) INTO integration_count FROM dev.user_integrations WHERE service_id = 'gmail';
...
DECLARE
    integration_count int;
BEGIN
    SELECT count(*) INTO integration_count FROM dev.user_integrations WHERE service_id = 'google_calendar';
...
DECLARE
    integration_count int;
BEGIN
    SELECT count(*) INTO integration_count FROM dev.user_integrations WHERE service_id = 'telegram';
...
DECLARE
    integration_count int;
BEGIN
    SELECT count(*) INTO integration_count FROM dev.user_integrations WHERE service_id = 'signal';
...
-- 8.2: Rename tables
ALTER TABLE dev.users RENAME TO users_old;
ALTER TABLE dev.users_new RENAME TO users;
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        ALTER TABLE dev.user_integrations
    ...
    RAISE NOTICE 'Recreated all foreign key constraints pointing to new users table';
END $$;
...
DECLARE
    ...
    new_count int;
BEGIN
    SELECT count(*) INTO old_count FROM dev.users_old;
    SELECT count(*) INTO new_count FROM dev.users;
...
COMMENT ON TABLE dev.users IS 'Core user profile data (integration data moved to user_integrations)';
COMMENT ON COLUMN dev.users.email IS 'User email address (unique identifier)';
COMMENT ON TABLE dev.users_old IS 'Backup of old users table (safe to drop after verification)';
...
CREATE OR REPLACE FUNCTION dev.get_user_integrations(p_user_id text)
RETURNS TABLE (
    integration_id uuid,
    service_name text,
    service_type text,
...
COMMENT ON FUNCTION dev.user_has_service IS 'Checks if user has an active integration for a service';
...
COMMENT ON FUNCTION dev.get_user_with_integrations IS 'Returns user with complete integration details as JSON. WARNING: Contains sensitive tokens!';
...
COMMENT ON FUNCTION dev.get_valid_token IS 'Returns token with expiration status for any service. Returns NULL if not found.';
...
COMMENT ON FUNCTION dev.get_gmail_token IS 'Returns Gmail token with expiration status. Wrapper for get_valid_token.';
...
COMMENT ON FUNCTION dev.get_calendar_token IS 'Returns Google Calendar token with expiration status. Wrapper for get_valid_token.';
...
COMMENT ON FUNCTION dev.get_telegram_token IS 'Returns Telegram credentials with status. Wrapper for get_valid_token.';
...
COMMENT ON FUNCTION dev.get_signal_token IS 'Returns Signal credentials with status. Wrapper for get_valid_token.';
...
COMMENT ON FUNCTION dev.has_valid_token IS 'Returns true if user has active, non-expired token for service.';
...
COMMENT ON FUNCTION dev.update_service_token IS 'Updates token with automatic expiration. Raises error if integration not found.';
...
COMMENT ON FUNCTION dev.update_gmail_token IS 'Updates Gmail token. Default 3600 seconds (1 hour).';
...
COMMENT ON FUNCTION dev.update_calendar_token IS 'Updates Calendar token. Default 3600 seconds (1 hour).';
...
COMMENT ON FUNCTION dev.expire_refresh_token IS 'Marks refresh token as expired. Raises error if integration not found.';
...
COMMENT ON FUNCTION dev.expire_gmail_refresh_token IS 'Marks Gmail refresh token as expired.';
...
COMMENT ON FUNCTION dev.expire_calendar_refresh_token IS 'Marks Calendar refresh token as expired.';

-- Helper function to get user_id by external integration ID
CREATE OR REPLACE FUNCTION dev.get_user_id_by_external_id(
    p_service_id text,
...
COMMENT ON FUNCTION dev.get_user_id_by_external_id IS 'Returns user_id for a given external service ID (e.g., telegram_id). Returns NULL if not found.';

-- Helper function to get full user by external integration ID
CREATE OR REPLACE FUNCTION dev.get_user_by_external_id(
    p_service_id text,
    p_external_user_id text
)
RETURNS jsonb
...
AS $$
    ...
    FROM dev.users u
    JOIN dev.user_integrations ui ON u.id = ui.user_id
...
COMMENT ON FUNCTION dev.get_user_by_external_id IS 'Returns full user record for a given external service ID. Returns NULL if not found.';

-- Convenience wrapper for Telegram
CREATE OR REPLACE FUNCTION dev.get_user_id_by_telegram_id(p_telegram_id text)
RETURNS text
LANGUAGE sql
STABLE
AS $$
    SELECT dev.get_user_id_by_external_id('telegram', p_telegram_id);
$$;

COMMENT ON FUNCTION dev.get_user_id_by_telegram_id IS 'Returns user_id for a telegram_id. Wrapper for get_user_id_by_external_id.';

CREATE OR REPLACE FUNCTION dev.get_user_by_telegram_id(p_telegram_id text)
RETURNS jsonb
LANGUAGE sql
STABLE
AS $$
    SELECT dev.get_user_by_external_id('telegram', p_telegram_id);
$$;

COMMENT ON FUNCTION dev.get_user_by_telegram_id IS 'Returns full user record for a telegram_id. Wrapper for get_user_by_external_id.';

-- Convenience wrapper for Signal
CREATE OR REPLACE FUNCTION dev.get_user_id_by_signal_id(p_signal_id text)
RETURNS text
LANGUAGE sql
STABLE
AS $$
    SELECT dev.get_user_id_by_external_id('signal', p_signal_id);
$$;

COMMENT ON FUNCTION dev.get_user_id_by_signal_id IS 'Returns user_id for a signal_source_uuid. Wrapper for get_user_id_by_external_id.';

CREATE OR REPLACE FUNCTION dev.get_user_by_signal_id(p_signal_id text)
RETURNS jsonb
LANGUAGE sql
STABLE
AS $$
    SELECT dev.get_user_by_external_id('signal', p_signal_id);
$$;
...
AS $$
    ...
    FROM (
        SELECT 
            ...
            tm.direction,
            tm.sender_id
        FROM dev.telegram_messages tm
        JOIN dev.user_integrations ui ON ui.external_user_id = tm.sender_id
        WHERE ui.user_id = p_user_id
...
COMMENT ON FUNCTION dev.get_user_telegram_messages IS 'Returns last N telegram messages for a user as JSONB array. Defaults to 8 inbound messages.';

-- Alternative function returning table format (for easier joins)
CREATE OR REPLACE FUNCTION dev.get_user_telegram_messages_table(
    p_user_id text,
    p_limit int DEFAULT 8,
    p_direction text DEFAULT 'inbound'
)
...
COMMENT ON FUNCTION dev.get_user_telegram_messages_table IS 'Returns last N telegram messages for a user as table. Defaults to 8 inbound messages. Use this for JOINs or when you need individual columns.';

-- Function to get all users with a specific service enabled
CREATE OR REPLACE FUNCTION dev.get_users_with_service(
    p_service_id text
)
RETURNS TABLE (
    id text,
...
COMMENT ON FUNCTION dev.get_users_with_service IS 'Returns all users who have a specific service enabled with valid tokens. Use service_id like gmail, google_calendar, telegram, signal.';

-- Convenience functions for common services
CREATE OR REPLACE FUNCTION dev.get_users_with_gmail()
RETURNS TABLE (
    id text,
    email text,
    display_name text,
...
STABLE
AS $$
    SELECT * FROM dev.get_users_with_service('gmail');
$$;

COMMENT ON FUNCTION dev.get_users_with_gmail IS 'Returns all users with Gmail integration enabled.';

CREATE OR REPLACE FUNCTION dev.get_users_with_calendar()
RETURNS TABLE (
    id text,
    email text,
    display_name text,
...
STABLE
AS $$
    SELECT * FROM dev.get_users_with_service('google_calendar');
$$;

COMMENT ON FUNCTION dev.get_users_with_calendar IS 'Returns all users with Google Calendar integration enabled.';

CREATE OR REPLACE FUNCTION dev.get_users_with_telegram()
RETURNS TABLE (
    id text,
    email text,
    display_name text,
...
STABLE
AS $$
    SELECT * FROM dev.get_users_with_service('telegram');
$$;

COMMENT ON FUNCTION dev.get_users_with_telegram IS 'Returns all users with Telegram integration enabled.';

CREATE OR REPLACE FUNCTION dev.get_users_with_signal()
RETURNS TABLE (
    id text,
    email text,
    display_name text,
...
STABLE
AS $$
    SELECT * FROM dev.get_users_with_service('signal');
$$;

COMMENT ON FUNCTION dev.get_users_with_signal IS 'Returns all users with Signal integration enabled.';

-- Function to get users with service + their integration details (JSONB)
CREATE OR REPLACE FUNCTION dev.get_users_with_service_details(
    p_service_id text
)
RETURNS jsonb
LANGUAGE sql
...
COMMENT ON FUNCTION dev.get_users_with_service_details IS 'Returns JSONB with users and their integration details for a specific service.';
...
BEGIN
    SELECT count(*) INTO total_users FROM dev.users;
    SELECT count(*) INTO total_integrations FROM dev.user_integrations;
    SELECT count(*) INTO gmail_count FROM dev.user_integrations WHERE service_id = 'gmail';
    SELECT count(*) INTO calendar_count FROM dev.user_integrations WHERE service_id = 'google_calendar';
    SELECT count(*) INTO telegram_count FROM dev.user_integrations WHERE service_id = 'telegram';
    SELECT count(*) INTO signal_count FROM dev.user_integrations WHERE service_id = 'signal';
...
--   5. Recreate the original FK constraints
...
-- 6. After 1-2 weeks of stability, DROP TABLE dev.users_old
...
-- AND u.id NOT IN (SELECT user_id FROM dev.user_integrations);
...
-- SELECT dev.update_gmail_token('user_123', 'ya29.new_token', 3600);
--
-- Update Calendar token with refresh token:
-- SELECT dev.update_calendar_token('user_123', 'ya29.new_token', 3600, '1//0g_refresh');
--
-- Generic token update:
-- SELECT dev.update_service_token('user_123', 'gmail', 'ya29.new_token', 3600);
--
-- Expire refresh token:
-- SELECT dev.expire_calendar_refresh_token('user_123');
--
-- Generic expire:
-- SELECT dev.expire_refresh_token('user_123', 'google_calendar');
--
